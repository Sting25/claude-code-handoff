...
- **History fallback showed BIND marker lines inside the reference-data block (#136).** An archived snapshot never binds, but the `SessionStart` history fallback printed its `<!-- HANDOFF_BIND_BEGIN -->` and `<!-- HANDOFF_BIND_END -->` lines verbatim (4 per load in the run above), which made its old fences look like a binding region. The fallback now drops those two marker lines, tolerating CRLF, and keeps the fences as reference text. The auto-rebuild path for a missing or corrupted current doc is unchanged; it is tracked separately in #104.
